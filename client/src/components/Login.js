...
export default function Login({ ws, onLogin }) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === "signup_success") setError("Signup success! Please login.");
  ...
  return (
    <div className="container">
      <h2>Login / Signup</h2>
      <input placeholder="Username" value={username} className="loinput" onChange={e => setUsername(e.target.value)} />
      <input placeholder="Password" type="password"  className="loinput" value={password} onChange={e => setPassword(e.target.value)} />
      <br />
